  ...
  <div class="card-body">
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="cardTabContent">
                            ...
                            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                              ...
                              <form action="./includes/excel_oportunidades.php" method="POST">
   ...
   $idUsuario = $_SESSION['idUser'];
     if ($_SESSION['rol'] == 1) {
      ...
      $sql = "SELECT
          (SELECT SUM(MRC) FROM sales WHERE tipo = 'PEN' AND idUsuario = '$idUsuario') AS sum_mrc_pen,
          (SELECT SUM(FCV) FROM sales WHERE tipo = 'PEN' AND idUsuario = '$idUsuario') AS sum_fcv_pen,
          (SELECT SUM(`One Shot`) FROM sales WHERE tipo = 'PEN' AND idUsuario = '$idUsuario') AS sum_one_shot_pen,
          (SELECT SUM(`One Shot`) FROM sales WHERE tipo = 'USD' AND idUsuario = '$idUsuario') AS sum_fcv_usd;";
  }
            ...
            <div class="col">
                <div class="kanban-board">
                    ...
                    foreach ($columns as $column) {
                        ...
                        echo '<h2 style="font-size: 18px;" >' . $column . '  </h2>';
                        if ($_SESSION['rol'] == 1) {
                        ...
                        $sql = "SELECT  fs.url, s.id,s.Oportunidad,cu.Company,s.Status,s.Priorit,s.MRC,u.usuario,s.Detalle,co.celular,s.`Expected Close`, co.nombre, s.FCV, s.`One Shot`, s.Producto, s.Propuesta FROM sales as s inner join file_sales as fs on s.id = fs.COD_idSales INNER JOIN customers as cu on cu.idCliente = s.idCustomer INNER JOIN contacts as co on co.idContacts = s.idContact INNER JOIN usuario as u on s.idUsuario = u.idusuario WHERE Status = '$column' and u.idusuario = '$idUsuario'";
                        }
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                              ...
                              if (strlen($oportunidad) > 40) {
                                  $oportunidad = substr($oportunidad, 0, 40) . '...';
                              }


                              echo '<div class="card mb-2" style="font-size: 15px;" id="item-' . $row['id'] . '">' .
                              '<div style="height: 320px;font-size: 12px;" class="card-body text-center">' .
                              '<h5 class="card-title">' . $oportunidad . '</h5>' .
                              '<p class="card-text">Company: ' . $company . '</p>' .
                              '<p class="card-text">MRC: ' . $row['MRC'] . '</p>' .
                              '<p class="card-text">'.
                              ' <span class="badge bg-success text-white badge-lg" style="font-size: 20px;">'. $row['usuario']  .'</span>'.
                              '</p>'.

                              '<center>';
                              if (verificarmostrarDatos(mostrarDatos(6), 3) != -1) {
                              echo '<a href="editar_sales1.php?id='.$row['id'].'" class="btn btn-primary"><i class="fas fa-pencil-alt"></i></a>' ;
                              }
                              if (verificarmostrarDatos(mostrarDatos(6), 4) != -1) {
                              echo '<form action="eliminar_sale.php?id=' . $row['id'] . '" method="post" class="confirmar d-inline">' .
                                  '<button class="btn btn-danger" type="submit"><i class="fas fa-trash-alt"></i></button>' .
                              '</form>' ;
                              }
                              echo '</center>'.
