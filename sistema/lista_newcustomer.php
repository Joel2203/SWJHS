			...
			<div class="table-responsive">
				<table class="mi-tabla" id="table">
					...
					<tbody>
						...
						$query = mysqli_query($conexion, "SELECT * FROM newcustomers");
						$result = mysqli_num_rows($query);
						if ($result > 0) {
							while ($data = mysqli_fetch_assoc($query)) { ?>
								<tr>
									<td><?php echo $data['idnewCliente']; ?></td>
									...
									<?php if ($_SESSION['rol'] == 1) { ?>
									<td>
										<a href="editar_customer.php?id=<?php echo $data['idnewCliente']; ?>" class="btn btn-success"><i class='fas fa-edit'></i></a>
										<form action="eliminar_customer.php?id=<?php echo $data['idnewCliente']; ?>" method="post" class="confirmar d-inline">
